...
def train(args):
    ...
    input1_tensor, input2_tensor = loadSingleData(data_path=args.path, img1_name=args.img1_name,
                                                  img2_name=args.img2_name)
    if torch.cuda.is_available():
    ...
    for epoch in range(start_epoch, start_epoch + args.max_iter):
        ...
        print("Training: Iteration[{:0>3}/{:0>3}] Total Loss: {:.4f} lr={:.8f}".format(current_iter, args.max_iter,
                                                                                       total_loss,
        ...
        if current_iter == 1:
            ...
            cv2.imwrite(args.path + 'before_optimization.jpg',
        ...
        if current_iter >= 4:
            if torch.abs(loss_list[current_iter - 4] - loss_list[current_iter - 3]) <= 1e-4 and torch.abs(
                    loss_list[current_iter - 3] - loss_list[current_iter - 2]) <= 1e-4 \
                ...
                cv2.imwrite(args.path + "iter-" + str(epoch - start_epoch + 1).zfill(3) + "_mesh.jpg",
