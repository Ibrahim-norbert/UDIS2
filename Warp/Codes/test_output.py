...
def test(args):
    ...
    test_loader = DataLoader(dataset=test_data, batch_size=args.batch_size, num_workers=1, shuffle=False,
                             drop_last=False)
    ...
    for i, batch_value in enumerate(test_loader):
        ...
        path = path_warp1 + str(i + 1).zfill(6) + ".tif"
        cv2.imwrite(path, final_warp1)
        path = path_warp2 + str(i + 1).zfill(6) + ".tif"
        cv2.imwrite(path, final_warp2)
        path = path_mask1 + str(i + 1).zfill(6) + ".tif"
        cv2.imwrite(path, final_warp1_mask * 255)
        path = path_mask2 + str(i + 1).zfill(6) + ".tif"
        ...
        path = path_ave_fusion + str(i + 1).zfill(6) + ".tif"
        cv2.imwrite(path, ave_fusion.astype(np.uint16))
